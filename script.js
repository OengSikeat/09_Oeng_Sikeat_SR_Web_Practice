...
searchForm.addEventListener("submit", (e) => {
    ...
    if (warriorSelect.value === "id") {
        if (Number(search) === 1) {
            ...
            error.innerHTML="";
            updateCat("https://pbs.twimg.com/media/GFdyZG8X0AADOZb.jpg", "Khalid",
                      "Khalid is a cat warrior who is a master of the sword and the bow.");
        }
        else if (Number(search) === 2) {
            contentElement?.classList.remove("hidden");
            error.innerHTML="";
            updateCat("https://us-tuna-sounds-images.voicemod.net/088e1aac-5559-4ed7-a563-1e66829610b6-1710041068109.png", "Usman",
                      "Usman is a cat warrior who is a master of the sword and the bow.");
        }
        else if (Number(search) === 3) {
            error.innerHTML="";
            contentElement?.classList.remove("hidden");
            updateCat("https://i.ytimg.com/vi/O08XTXJSOtI/maxresdefault.jpg", "Muhammad",
